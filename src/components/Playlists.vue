<template>
    ...
    <table v-else>
        <tr v-for="playlist in playlists" :key="playlist.id">
            ...
            <td class="rename" v-if="editor_active"><i class="fa fa-edit" @click="$store.commit('renamePlaylist', playlist)"/></td>
            <td class="delete" v-if="editor_active"><i class="fa fa-trash-o" @click="$store.commit('deletePlaylist', playlist)"/></td>
        </tr>
    </table>
</template>
